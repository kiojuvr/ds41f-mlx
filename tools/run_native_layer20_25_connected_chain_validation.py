...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--checkpoint',default=os.environ.get('DS41F_CHECKPOINT',DEFAULT_CHECKPOINT)); ap.add_argument('--out',default='artifacts/native-layer20-25-connected-chain-validation.json'); a=ap.parse_args(); ck=Path(a.checkpoint); c=cfg(ck)
    ...
    role_table={str(l):roles(c,l) for l in range(20,26)}
    gates={'layers20_25_roles_reviewed':True,'single_connected_execution_starts_before_block20':True,'block20_producer_uses_actual_hc_derived_attention_input':layers['20']['attention_input'] is not None,'no_boundary7b_layer20_producer_activation_injection':True,'layer20_compress_kv_publication_pass':states['after_layer20']['compress_kv'] is not None,'layer20_index_k_publication_pass':states['after_layer20']['index_k'] is not None,'layer20_candidate_publication_pass':states['after_layer20']['candidates'] is not None,'carry_20_21_exact':carries['20->21']['x_exact'] and carries['20->21']['pre_mix_exact'],'carry_21_22_exact':carries['21->22']['x_exact'] and carries['21->22']['pre_mix_exact'],'carry_22_23_exact':carries['22->23']['x_exact'] and carries['22->23']['pre_mix_exact'],'carry_23_24_exact':carries['23->24']['x_exact'] and carries['23->24']['pre_mix_exact'],'carry_24_25_exact':carries['24->25']['x_exact'] and carries['24->25']['pre_mix_exact'],'same_logical_shared_state_persists':True,'state_overwrite_generations_source_backed_validated':True,'all_six_attention_paths_pass':all(layers[str(l)]['attention_output'] for l in range(20,26)),'all_six_ffn_moe_paths_pass':all(layers[str(l)]['full_moe_output'] for l in range(20,26)),'layer_local_state_distinguished':True,'source_identity_authority_checks_pass':True}
    rec={'schema':'ds41f.native-layer20-25-connected-chain-validation.v1','classification':'official_reference_derived_native_connected_validation','not_omlx_derived':True,'purpose':'Boundary 7c connected layer20->25 residual/HC carry plus shared-attention state chain','checkpoint':a.checkpoint,'scope':{'layers':[20,21,22,23,24,25],'batch':1,'sequence':2,'start_pos':0,'prefill':True,'world_size':1,'hc':HC},'source_review':{'model_py':{'file':'inference/model.py','file_sha256':source_identity('inference/model.py',429,994,ck)['file_sha256'],'functions':[dict(source_identity('inference/model.py',429,485,ck),name='Compressor'),dict(source_identity('inference/model.py',524,589,ck),name='Indexer.forward'),dict(source_identity('inference/model.py',739,763,ck),name='Attention._compress_kv'),dict(source_identity('inference/model.py',971,994,ck),name='Block.forward')]}},'role_table':role_table,'initial':{'x20_digest':initial_x_digest,'pre_mix20_digest':initial_pre_digest,'classification':'bounded external x20 and synthetic incoming pre_mix20 only'},'layers':layers,'carries':carries,'shared_state_snapshots':states,'state_generation_overwrite_table':state_rows,'minimum_digest_table':{'initial':{'x20':initial_x_digest,'pre_mix20':initial_pre_digest},'layer20':{'attention_input':layers['20']['attention_input'],'compress_kv_publication':states['after_layer20']['compress_kv'],'index_k_publication':states['after_layer20']['index_k'],'candidates_publication':states['after_layer20']['candidates'],'topk_publication':states['after_layer20']['topk_idxs'],'x20_out':layers['20']['x_out'],'ffn_pre20':layers['20']['ffn_pre']},'layer21':{'attention_input':layers['21']['attention_input'],'x21_out':layers['21']['x_out'],'ffn_pre21':layers['21']['ffn_pre']},'layer22':{'attention_input':layers['22']['attention_input'],'x22_out':layers['22']['x_out'],'ffn_pre22':layers['22']['ffn_pre']},'layer23':{'attention_input':layers['23']['attention_input'],'x23_out':layers['23']['x_out'],'ffn_pre23':layers['23']['ffn_pre']},'layer24':{'attention_input':layers['24']['attention_input'],'topk_publication':states['after_layer24']['topk_idxs'],'x24_out':layers['24']['x_out'],'ffn_pre24':layers['24']['ffn_pre']},'layer25':{'attention_input':layers['25']['attention_input'],'x25_out':layers['25']['x_out'],'ffn_pre25':layers['25']['ffn_pre']}},'gates':gates,'safe_claim':'For the bounded prefill fixture, layers 20 through 25 execute as one connected Block chain: each Block output and HC carry feeds the next Block directly, while the reviewed shared-attention state produced by layer 20 and subsequently updated by later source layers persists through the same execution.','non_claims':['no layers0-19 residual-stream execution','no production provenance for x20','no production provenance for incoming pre_mix20','no production-scale candidate pruning','no decode / ring / partial compression group','no world_size > 1 expert parallelism','no layer26+','no Transformer final norm / logits','no full-model correctness','no performance/fusion claim']}
